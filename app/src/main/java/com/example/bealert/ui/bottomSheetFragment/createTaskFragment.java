package com.example.bealert.ui.bottomSheetFragment;
...
public class createTaskFragment extends BottomSheetDialogFragment {
    ...
    public void setTaskId(int taskId, boolean isEdit, setRefreshListener setRefreshListener, Main2Activity activity) {
        this.taskId = taskId;
        this.isEdit = isEdit;
   ...
   /* public createTaskFragment.setRefreshListener getSetRefreshListener() {
        return setRefreshListener;
    }*/
    ...
    public void createAnAlarm() {
        try {
            ...
            PendingIntent pendingIntent = PendingIntent.getBroadcast(getContext(),count, alarmIntent, PendingIntent.FLAG_IMMUTABLE);
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                alarmManager.setAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, cal.getTimeInMillis(), pendingIntent);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    alarmManager.setExact(AlarmManager.RTC_WAKEUP, cal.getTimeInMillis(), pendingIntent);
                    Toast.makeText(getActivity(), "Alarm 0 been added", Toast.LENGTH_SHORT).show();

                } else {
                    alarmManager.set(AlarmManager.RTC_WAKEUP, cal.getTimeInMillis(), pendingIntent);
                    Toast.makeText(getActivity(), "Alarm 1 been added", Toast.LENGTH_SHORT).show();
                }
                count ++;

                PendingIntent intent = PendingIntent.getBroadcast(getContext(), count, alarmIntent, PendingIntent.FLAG_IMMUTABLE);
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    ...
                    Toast.makeText(getActivity(), "Alarm 3 been added", Toast.LENGTH_SHORT).show();
                } else {
                    alarmManager.set(AlarmManager.RTC_WAKEUP, cal.getTimeInMillis() - 600000, intent);
                    Toast.makeText(getActivity(), "Alarm 4 been added", Toast.LENGTH_SHORT).show();
                }
                count ++;
            }
        } catch (Exception e) {
